...
namespace Ass_Zed.Common.DamageIndicator
{
    ...
    public class DamageIndicator : PluginModel
    {
        ...
        {
            if (!Misc.IsChecked(DrawMenu, "drawDmg")) return;
            ...
            foreach (var aiHeroClient in EntityManager.Heroes.Enemies)
            {
                ...
                var damage = (_barLength) *
                             ((DmgLib.PossibleDamage(aiHeroClient) / aiHeroClient.MaxHealth) > 1
                                 ? 1
                                 : (DmgLib.PossibleDamage(aiHeroClient) / aiHeroClient.MaxHealth));
